...
const ScrollingCarousel = ({ items }) => {
  ...
  return (
    <Container fluid style={{ backgroundColor: "#5E5E5E", position: 'relative' }}>
      ...
      <div key={index} className="carousel-item">
        {chunk.map((person, i) => (
          <div key={i} style={{ position: 'relative' }}>
            ...
            <div>
              <h3 className="fst-italic" style={{ fontSize: '1.3rem', color: 'white' }}>{person.name}</h3>
              <p className="fst-italic fw-lighter" style={{ fontSize: '1.1rem', color: 'white' }}>{person.bio}</p>
            </div>
          </div>
        ))}
      </div>
